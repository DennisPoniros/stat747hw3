...
\textbf{Insights:} In the hard-margin setting (separable data), the dual remains \textbf{well-posed with finite optimal solution}.
...
\item \textbf{Hard-margin case (linearly separable):} 
   ...
   \item Optimal solution yields finite $\alpha_i^*$ values
   \item Only margin support vectors have $\alpha_i^* > 0$
   \end{itemize}

\item \textbf{The real issue:} The missing $\alpha_i \leq C$ constraint becomes problematic for \textbf{non-separable} data, where it would lead to unbounded $\alpha_i$ for misclassified points.
\end{enumerate}
...
SVR (Polynomial) & 0.0145 & 0.0115 & 0.470 \\
Linear Regression & 0.0853 & 0.0692 & -0.834 \\
\hline
\end{tabular}
\end{center}
...
\item Fastest training among competitive models (0.014s)
...
\section{Excercise 4: The Master’s Reflection (10 points)}
...
def tune_svr(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    results['linear'] = {
        ...
        'n_support': svr_linear.best_estimator_.n_support_
    }
    ...
    results['rbf'] = {
        ...
        'test_r2': r2_score(y_test, test_pred),
        'train_time': train_time,
        'n_support': svr_rbf.best_estimator_.n_support_
    }
