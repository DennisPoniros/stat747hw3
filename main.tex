...
\item \textbf{Conclusion:} Alice's formulation is only valid when implicitly restricting to the hard-margin regime with perfectly separable data. For general soft-margin SVM, the constraint $\alpha_i \leq C$ is essential.
\end{enumerate}
...
\hline
Linear Regression & 0.0853 & -19.53 \\
Decision Tree (unpruned) & 0.0154 & 0.33 \\
\hline
\end{tabular}
\end{center}
...
\textbf{Residual Analysis:} Examination of residuals for the best-performing SVR model reveals:
\begin{itemize}
\item Residuals approximately normally distributed (confirmed via Q-Q plot)
\item Mean residual near zero (unbiased predictions)
\item No systematic patterns in residual vs. predicted values plot
\item Slight heteroscedasticity at extreme predicted values
\item Residual standard deviation: 0.0087 (consistent with RMSE)
\end{itemize}
...
\begin{itemize}
\item \textbf{YrsOpen}: Positive relationship with response, with steeper increase for values $> 0.4$
\item \textbf{EquipInv}: Non-linear relationship showing diminishing returns at high values
\item \textbf{Confucian}: Threshold effect around 0.5, with minimal impact below this value
\item These patterns align with economic intuition about productivity and institutional effects
\end{itemize}
...
\textbf{Visual inspection of misclassified examples} (see Appendix Figure) shows that:
\begin{enumerate}
...
\item Model confidence (decision function values) is typically lower for misclassified examples
\end{enumerate}
...
\begin{center}
\textbf{Video Link:} \url{https://[TO_BE_PROVIDED]}
\end{center}
